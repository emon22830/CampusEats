...
class VendorRepository
{
    ...
    /** Every vendor regardless of status/active state - admin management view. */
    ...
    /** True if any order still references this vendor (blocks a hard delete). */
    public function hasOrders(int $id): bool
    {
        $stmt = $this->db->prepare('SELECT 1 FROM orders WHERE vendor_id = :id LIMIT 1');
        $stmt->execute(['id' => $id]);
